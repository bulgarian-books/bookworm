...
select = Reacto::PG.select(settings.connection, 'select_publishers')
  .map { |row| row['code'] }
  .delay_each(0.4)
  .flat_map do |publisher_code|
    Reacto::HTTP.get(
      'http://www.booksinprint.bg/Publication/Search?SearchCriteria' \
      "=PublisherCode%3A#{publisher_code}%3A"
    ).wrap(publisher_code: publisher_code)
  end
  .map { |data| data.to_h.merge(value: Nokogiri::HTML(data.value)) }
  .select { |data| !data[:value].css('div#resultsContainer').empty? }
  .map do |data|
    pages = data[:value].css('.pageing .right').text.strip
    ...
    requests = (2..pages).map do |page|
      Reacto::HTTP.get(
        'http://www.booksinprint.bg/Publication/Search?SearchCriteria' \
        "=PublisherCode%3A#{publisher_code}%3A&page=#{page}"
      ).retry(3).map { |v| Nokogiri::HTML(v) }
    end

    ([Reacto::Trackable.value(data[:value])] + requests).map do |trackable|
      trackable
  ...
  .flatten.flat_map { |trackable| trackable }

consumer = ->(value) do
  p "#{value[:title]} : #{value[:publisher_code]}"
  ...
  settings.connection.transaction do |connection|
    ...
    record = connection.exec_prepared(
      'select_publisher_by_code', [value[:publisher_code]]
    ).first
...
subscription = select.on(
  value: consumer,
  error: ->(e) { raise e },
  close: ->() { p 'Done' }
)

select.await(subscription)
